...
public class AddPermissionWithContextCommand implements CommandExecutor {
    private final CookbookPlugin plugin;
    private final LuckPerms luckPerms;

    public AddPermissionWithContextCommand(CookbookPlugin plugin, LuckPerms luckPerms) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        Node node = Node.builder(permission).withContext(DefaultContextKeys.SERVER_KEY, "survival").build();
        ...
        this.luckPerms.getUserManager().modifyUser(player.getUniqueId(), (User user) -> {
            ...
            if (result.wasSuccessful()) {
                sender.sendMessage(ChatColor.RED + user.getUsername() + " now has permission " + permission + " in the survival server");
            } else {
